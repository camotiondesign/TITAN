#!/usr/bin/env python3
"""
Migrate content from TITANVERSE campaigns to published posts.
...
def is_template_content(content):
    """Check if content is a template/placeholder"""
...
def migrate_content(campaign_path, post_path, force=False):
    ...
    if not post_alt.exists():
        ...
        if size == 0 or size < 10:  # Empty or just whitespace
            ...
            try:
                current_content_full = post_alt.read_text(encoding='utf-8')
                current_content = current_content_full.lower()
                current_is_template = is_template_content(current_content_full)
                if current_is_template:
                    needs_migration = True  # Always replace templates
            except:
                needs_migration = True
    ...
    if transcripts:
        ...
        # Check if transcript is a template - if so, skip it and use alt-text sources instead
        if is_template_content(transcript_content):
            has_real_transcript = False
            migrations.append(f"Skipping template transcript: {transcript_to_use['name']}")
        else:
            has_real_transcript = True
            
            # Check if current content is clearly wrong (e.g., short transcript for longform post)
            should_write = True
            if current_content and not needs_migration and not current_is_template:
            ...
            if should_write or needs_migration or current_is_template:
                ...
                return migrations  # Real transcripts take priority
            elif not needs_migration:
                return migrations  # Already has content and it seems correct
    
    # 2. Check for alt text sources (use if no real transcript or if current content is template)
    if not has_real_transcript or needs_migration or current_is_template:
        alt_sources = find_alt_text_sources(campaign_path)
        if alt_sources:
            # Prioritize single-image over carousel (single-image usually has better alt-text)
            # Sort by priority: single-image > poll > blog > carousel > linkedin-alt
            priority_order = {'single-image': 1, 'poll': 2, 'blog': 3, 'carousel': 4, 'linkedin-alt': 5}
            alt_sources_sorted = sorted(alt_sources, key=lambda x: priority_order.get(x['type'], 99))
            
            for alt_source in alt_sources_sorted:
                try:
                    content = alt_source['path'].read_text(encoding='utf-8')
                    # Skip if this is also a template
                    if is_template_content(content):
                        migrations.append(f"Skipping template {alt_source['type']} content: {alt_source.get('name', alt_source['path'].name)}")
                        continue
                    if content.strip():  # Only write if content is not empty
                        ...
                        return migrations  # Found real content, stop here
...
def main():
    campaigns_base = Path('campaigns/TITANVERSE')
    posts_base = Path('posts/titanverse/published')
    ...
    # Print summary
    print("=" * 70)
    print("TITANVERSE CAMPAIGN TO POST CONTENT MIGRATION SUMMARY")
    print("=" * 70)
